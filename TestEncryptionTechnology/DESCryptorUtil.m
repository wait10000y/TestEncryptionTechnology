...
+(NSString *)AES128Encrypt:(NSString *)plainText WithKey:(NSString *)gkey withIv:(NSString *)gIv
{
  ...
  if (cryptStatus == kCCSuccess) {
    ...
    return [self base64EncodedStringFrom:resultData];
  }
  free(buffer);
  return nil;
}

+(NSString *)AES128Decrypt:(NSString *)encryptText WithKey:(NSString *)gkey withIv:(NSString *)gIv
{
  char keyPtr[kCCKeySizeAES128 + 1];
  memset(keyPtr, 0, sizeof(keyPtr));
  [gkey getCString:keyPtr maxLength:sizeof(keyPtr) encoding:NSUTF8StringEncoding];
